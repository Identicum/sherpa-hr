...
def add():
    ...
    if request.method == 'POST':
        ...
        current_app.logger.debug("Attempting to add employee for person: %s", person_id)
    ...
    return render_template('employee_form.html', action='Add', persons=[(p.id, p.first_name, p.last_name) for p in persons], positions=[(p.id, p.name) for p in positions], managers=[(p.id, p.first_name, p.last_name) for p in managers if p])
...
def update(employee_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('employees.list'))
    return render_template('employee_form.html', action='Update', employee=emp, employee_id=employee_id, positions=[(p.id, p.name) for p in positions], managers=[(p.id, p.first_name, p.last_name) for p in managers if p])
