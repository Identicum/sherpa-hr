...
def add():
    ...
    return render_template('contractor_form.html', action='Add', persons=[(p.id, p.first_name, p.last_name) for p in persons], departments=[(d.id, d.name) for d in departments])
...
def update(contractor_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('contractors.list'))
    return render_template('contractor_form.html', action='Update', contractor=contractor, contractor_id=contractor_id, departments=[(d.id, d.name) for d in departments])
